...
def test_restaurant_not_found_error(mock_scrape_reviews):
    ...
    assert (
        ...
        == ErrorResponse(success=False, error="NonExistentRestaurant").dict()
    )
...
def test_reviews_not_found_warning(mock_scrape_reviews):
    ...
    with pytest.raises(HTTPException) as exc_info:
        response = client.get(
            "/scrape-justeat-reviews?restaurant_name=RestaurantWithNoReviews"
        )

        assert exc_info.value.status_code == status.HTTP_204_NO_CONTENT
        assert (
            response.headers["X-Warning-Message"]
            == "There are still no reviews for this restaurant 'RestaurantWithNoReviews'"
        )
        assert (
            exc_info.value.detail
            == ErrorResponse(
                success=False,
                error="ReviewsNotFoundWarning: There are still no reviews for this restaurant 'RestaurantWithNoReviews'",
            ).dict()
        )
